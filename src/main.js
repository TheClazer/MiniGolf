...
strokesEl.style.position='fixed'; strokesEl.style.left='12px'; strokesEl.style.top='12px';
strokesEl.style.padding='6px 10px'; strokesEl.style.background='rgba(0,0,0,0.5)';
strokesEl.style.color='#fff'; strokesEl.style.fontFamily='monospace'; strokesEl.style.zIndex=9999;
strokesEl.innerText = `Strokes: ${strokes}`; document.body.appendChild(strokesEl);
...
loader.load('/assets/hole1.glb', gltf => {
  ...
  gltf.scene.traverse(obj => {
    if (obj.isMesh) {
      // treat all meshes as colliders (heuristic)
...
/* AIMING (unchanged behavior) */
let isAiming=false, aimStart=new THREE.Vector3(), aimLine=null, activePointerId=null;
const maxDrag = 1.5;
...
renderer.domElement.addEventListener('pointerdown', e=>{
  ...
  isAiming=true; activePointerId=e.pointerId; controls.enabled=false;
...
renderer.domElement.addEventListener('pointermove', e=>{
  ...
  if(isAiming && ballMesh){
    ...
    if(!aimLine){ const geom=new THREE.BufferGeometry().setFromPoints([p1,p2]); aimLine=new THREE.Line(geom, aimLineMaterial); scene.add(aimLine); }
    else { aimLine.geometry.setFromPoints([p1,p2]); aimLine.geometry.attributes.position.needsUpdate = true; }
...
renderer.domElement.addEventListener('pointerup', e=>{
  if(activePointerId!==null && e.pointerId!==activePointerId) return;
  if(isAiming && ballMesh){
    isAiming=false; activePointerId=null; controls.enabled=true;
    ...
    strokes += 1; strokesEl.innerText = `Strokes: ${strokes}`; grounded=false;
  } else {
    activePointerId=null; isAiming=false; controls.enabled=true;
    if(aimLine){ scene.remove(aimLine); aimLine.geometry.dispose(); aimLine=null; aimLineMaterial=new THREE.LineBasicMaterial({ color:0xffff00, linewidth:2 }); }
  }
});
...
function closestPointOnTriangle(p, a, b, c, out){
  ...
  const d1 = ab.dot(ap);
  const d2 = ac.dot(ap);
  if (d1 <= 0 && d2 <= 0) { return out.copy(a); } // barycentric (1,0,0)
  bp.subVectors(p,b);
  const d3 = ab.dot(bp);
  const d4 = ac.dot(bp);
  if (d3 >= 0 && d4 <= d3) { return out.copy(b); } // (0,1,0)
  const vc = d1*d4 - d3*d2;
  if (vc <= 0 && d1 >= 0 && d3 <= 0) {
    const v = d1 / (d1 - d3);
    return out.copy(ab).multiplyScalar(v).add(a); // edge AB
  }
  cp.subVectors(p,c);
  const d5 = ab.dot(cp);
  const d6 = ac.dot(cp);
  if (d6 >= 0 && d5 <= d6) { return out.copy(c); } // (0,0,1)
  const vb = d5*d2 - d1*d6;
  if (vb <= 0 && d2 >= 0 && d6 <= 0) {
    const w = d2 / (d2 - d6);
    return out.copy(ac).multiplyScalar(w).add(a); // edge AC
  }
  const va = d3*d6 - d5*d4;
  if (va <= 0 && (d4 - d3) >= 0 && (d5 - d6) >= 0) {
    const w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
    return out.copy(b).sub(c).multiplyScalar(w).add(c); // edge BC
  }
  // inside face region. use barycentric coords
...
/* Resolve sphere vs mesh by iterating triangles */
  ...
  if (index){
    ...
    for (let i=0; i<idx.length; i+=3){
      ...
      if (distSq < BALL_RADIUS*BALL_RADIUS){
        ...
        // if triNormal points up strongly treat as ground contact
        if (triNormal.y > 0.6){
          // snap to triangle plane to avoid teleporting over top
          const planePoint = v0;
          const toCenter = ballMesh.position.clone().sub(planePoint);
          ...
          // remove normal component only and zero vertical
          const vNormalComp = triNormal.clone().multiplyScalar(velocity.dot(triNormal));
          velocity.sub(vNormalComp);
          velocity.y = 0;
          grounded = true;
        } else {
          // side collision: reflect velocity on triangle normal
        ...
        // we handle first collision triangle and break to avoid multiple corrections in same frame
        break;
      }
    }
  } else {
...
function physicsStep(dt){
  ...
  // ground raycast for reliable ground snapping before gravity
  let hitGround = null;
  if(courseScene){
  ...
  for(const mesh of colliderObjects){
    ...
    // break early if collision handled this frame and velocity reversed strongly
  }
